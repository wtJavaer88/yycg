...
public class UserAction {
	...
	public @ResponseBody DataGridResultInfo queryuser_result(SysuserQueryVo sysuserQueryVo, int page, // 页码
			int rows// 每页显示个数
	) throws Exception {
	...
	public @ResponseBody SubmitResultInfo addsysusersubmit(SysuserQueryVo sysuserQueryVo) throws Exception {
		 ...
		 * (Exception e) { //输出异常信息 e.printStackTrace(); //对应异常信息进行解析 // message
		 * = e.getMessage(); // type=1;//失败 //解析系统自定义异常 if(e instanceof
		 * ExceptionResultInfo){ //抛出的是系统自定义异常 resultInfo =
		 * ((ExceptionResultInfo)e).getResultInfo(); }else{ //重新构造“未知错误”异常
